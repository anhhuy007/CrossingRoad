...
bool LeaderBoard::OnCreate()
{
	...
	bg = Image("Screen\\leaderboardScreen\\img\\leaderboardBg.sprite");
	item = Image("Screen\\leaderboardScreen\\img\\item.sprite");
...
bool LeaderBoard::OnUpdate(float elapsedTime)
{
	...
	game->RenderSprite(title, { 60,10 });
	game->RenderSprite(text1, { 30,30 });
	game->RenderSprite(text2, { 220,30 });
	game->RenderSprite(medal1, { 10,60 });
	game->RenderSprite(medal2, { 10,90 });
	game->RenderSprite(medal3, { 10,120 });
	
	short left = 30, top = 60;
	for (int i = 0; i < 5; i++)
	{
		game->RenderSprite(item, { left,top });
		game->RenderSprite(item, { short(left+220),top });
		top += 30;
	}
